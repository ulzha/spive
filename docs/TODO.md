...
- [ ] Visualize workload logs (and metrics) somehow in between events as well... or just link to Stackdriver? Champion golden signals, aligned to timeline (with its warning and error hues), embedded from 3rd party sources like that? Should have a way to view logs sorted by event time? Example with Open Telemetry and some of its supporting receivers? Grafana Mimir
