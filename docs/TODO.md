...
- [ ] How will relationships work to support point-in-time consistent operations on all child entities?
...
- [ ] Support distributed synchronization of snapshot workloads across shards, if useful?
...
- [ ] An example CustomBlueGreenDeployment application showcasing how GHE events and Spīve API enables custom infrastructure automation for orchestrating your own application(s) deployments and their configurations. Generally useful for fleet management, ad-hoc experimentation, incident handling etc.
...
- [ ] Tolerate hacks/poor man's workarounds (nondeterministic processes, unmanaged/expiring streams, etc) but visualize clearly in Comic Sans (also opt-outs from automatic optimization?) (also suboptimal ones that can't deploy within 15 minutes? unless it's Spīve's fault)
