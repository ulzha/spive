...
- [ ] Separate SpiveInstaDebug application? Caching small samples of events for frontend for immediate display. A user doing error troubleshooting would thus perhaps see the data values needed without waiting for a full page from storage or from search backend. But keep order indication unmistakable
