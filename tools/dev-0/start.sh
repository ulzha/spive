...
until
  curl -X PUT -d '{"threadGroup": {"name": "foo", "artifactUrl": "file:///app/target/spive-0.0.1-SNAPSHOT.jar", "mainClass": "io.ulzha.spive.app.lib.SpiveInstance$Main", "args": ["io.ulzha.spive.core.LocalFileSystemEventStore;", "93ff4295-5a8c-4181-b50b-3d7345643581", "io.ulzha.spive.core.LocalFileSystemEventStore;", "93ff4295-5a8c-4181-b50b-3d7345643581", "dev-0", "event loop only"]}}' -i http://localhost:8079/api/v0/thread_groups
do echo "retrying in 1 s"; sleep 1; done

# 5. expect SpiveDevBootstrap to checkpoint to the end of its log and launch SpiveDev0
until
  heartbeat=$(curl http://localhost:8079/api/v0/thread_groups/foo/heartbeat)
...
until
  heartbeat=$(curl http://localhost:8080/api/v0/thread_groups/ff4726aa-9f71-49e2-b139-d71d780a817c/heartbeat)
  echo "$heartbeat" | jq -r .checkpoint | grep "#3"
do echo "retrying in 1 s"; sleep 1; done

# 9. print instructions, keep following the output of each container
echo "run tools/dev-0/stop.sh to stop"
