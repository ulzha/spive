 ...
 */
class BasicRunnerResource implements RouteProvider {
  private static final Logger LOG = LoggerFactory.getLogger(BasicRunnerResource.class);
    ...
    Stream<Route<AsyncHandler<Response<ByteString>>>> routes =
        Stream.of(
            Route.sync("PUT", "/thread_groups", context -> runThreadGroup(context.request()))
                .withMiddleware(BasicRunnerResource::serialize)
                .withDocString(
