 ...
 * (Comprehensive log collection is be configured separately, but we find it most beneficial for
 * productivity if the platform UI also keeps at least a sample of failure stacktraces available at
 * your fingertips for troubleshooting purposes.)
 *
 ...
 *
 * <p>(Ugly. Wonder if we can hide ThreadRunnerClient and Umbilicus, make it just Umbilical on both
 * ends...)
 */
public class Umbilical {
  ...
  // TODO FrontendClient/UiWebsocketClient? For graphing arrivals with minimum delay on focus...
  // With a specialized endpoint and a Javascript side, so UI can stream directly from the runner
  // Lens?
