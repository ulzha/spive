...
import io.ulzha.spive.threadrunner.util.jackson.ThreadRunnerModule;
import java.io.File;
...
class ThreadRunnerResource implements RouteProvider {
  ...
  public Stream<Route<AsyncHandler<Response<ByteString>>>> routes() {
    Stream<Route<AsyncHandler<Response<ByteString>>>> routes =
        Stream.of(
            ...
            // The following endpoint provides a best-effort way of error signaling from the event
            // handlers and workloads, with very limited retention and throughput (to prioritize
            // business logic).
            // The built in error signaling probably should not be developed further than a basic
            // enabler of KTLO and autoscaling, with minimal dependencies, and an enabler of
            // extremely fast and adequately structured error information propagation to UI in a few
            // most common troubleshooting scenarios.
            // Production environments would typically additionally log to a commodity async logging
    ...
    // TODO a separate endpoint with increased verbosity, to analyze for scaling purposes?
    // TODO a dump endpoint for seeing thread tree, and classloaders with their loaded classes
  ...
  private Response<String> runThreadGroup(final Request rawRequest) {
    ...
    try {
      request = objectMapper.readValue(requestJson, RunThreadGroupRequest.class);
    } catch (IOException e) {
  ...
  private Response<String> getThreadGroups() {
    ...
    return Response.forPayload(responsePayload);
  }
  ...
  private Response<String> getThreadGroupHeartbeat(final String name) {
    ...
    if (record == null) {
      ...
      final GetThreadGroupHeartbeatResponse response =
          GetThreadGroupHeartbeatResponse.create(record.umbilical);
      try {
        return Response.forStatus(Status.OK).withPayload(objectMapper.writeValueAsString(response));
      } catch (JsonProcessingException e) {
